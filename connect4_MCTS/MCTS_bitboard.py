...
class MCTS:
    ...
    def __init__(self, max_iter = 20000, C = 10, selection_method = 'random', player_type = 'MCTS', 
                 iterations_per_simulation = 1, max_runtime = 5, cap_method = 'iter'):
    ...
    def run_mcts(self, state):
        ...
        if self.player_type =='random':
            return random.choice(self.bitboard.get_moves())


        self.reset()
        self.root_state = copy.deepcopy(state)
        self.bitboard.set_state(state)
        self.main_player = state[2]
    ...
    def expand(self, parent):
        ...
        parent.children = {move: Node(move, parent, C = self.c) for move in self.bitboard.get_moves()}
